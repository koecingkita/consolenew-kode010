...
function TextRich() {
  ...
  const toggleList = (listType) => {
    ...
    while (currentList && currentList !== editorRef) {
    ...
    if (currentList && (currentList.tagName === 'UL' || currentList.tagName === 'OL')) {
      ...
      let block = container;
      while (block && block !== editorRef &&
             !['P', 'DIV', 'H1', 'H2', 'H3', 'H4', 'H5', 'H6', 'BLOCKQUOTE'].includes(block.tagName)) {
      ...
      if (block && block !== editorRef) {
        ...
        // If no block found, create a new list item
        const li = document.createElement('li');
        li.innerHTML = '<br>';
        list.appendChild(li);

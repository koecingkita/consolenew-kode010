...
const UPDATE = KategoriService.update;

function UpdateKategori(props) {
  const handleClose = () => {
    props.onClose();
  }

  const [title, setTitle] = createSignal(props.itemName.label);
  const [slug, setSlug] = createSignal(props.itemName.slug);
  const [desc, setDesc] = createSignal(props.itemName.excerpt);
    ...
    <div class='flex min-h-full items-center justify-center p-4'>
      <div class="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg" onClick={(e) => e.stopPropagation()} >
        <div class="px-4 pt-5 sm:p-6 sm:pb-4">
            <label class='font-semibold text-gray-900/80 uppercase'>Update Kategori Produk</label>

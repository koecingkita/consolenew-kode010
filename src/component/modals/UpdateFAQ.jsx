...
const UPDATE = FAQService.update;
...
function UpdateFAQ(props) {
  ...
  const [quest, setQuest] = createSignal(props.itemName.question);
  const [answer, setAnswer] = createSignal(props.itemName.answer);
    ...
    <div class='flex min-h-full items-center justify-center p-4'>
      <div class='relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg' onClick={(e) => e.stopPropagation()}>
        ...
        <div class="px-4 pt-5 sm:p-6 sm:pb-4">
            <label class='font-semibold text-gray-900/80 uppercase'>Update FAQ</label>
            <p class='mt-4 text-gray-800/70 text-sm'>
              FAQ berisi kumpulan pertanyaan dan jawaban yang sering diajukan untuk membantu pengguna mendapatkan informasi dengan cepat
            </p>
        </div>
